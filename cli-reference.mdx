...
Snapshots let you save the state of a running instance and restore it later. For detailed usage, see [Using Snapshots](/guides/using-snapshots).
...
This opens an interactive menu to select a running instance and name your snapshot. You can also use flags:

```
tnr snapshot create --instance-id 0 --name my-snapshot
```
...
```
tnr snapshot delete my-snapshot
```

### Restore from a Snapshot

When creating a new instance, your snapshots appear alongside templates in the selection menu. Choose a snapshot to create an instance with your saved state.
...
<Note>
  Restoring from a snapshot may take about 10 minutes per 100GB of data.
</Note>
