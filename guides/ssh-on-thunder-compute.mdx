...
Because Thunder Compute instances use [ephemeral storage](/guides/using-ephemeral-storage), the quickest fix is often to back up any critical files, delete the instance, and create a new one. This gives you a fresh environment without lingering SSH state.
