...
| `create_instance` | Create a new GPU instance (specify GPU type, GPU count, template, etc.) |
| `delete_instance` | Delete an instance (irreversible) |
| `modify_instance` | Change instance config (GPU type, GPU count, vCPUs, disk) |
...
| Prompt | Description |
|--------|-------------|
| `create-dev-instance` | Set up a GPU instance with sensible defaults |
