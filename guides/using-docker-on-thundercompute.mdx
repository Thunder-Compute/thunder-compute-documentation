...
<Warning>
  GPU support will not work out of the box on Production instances. Because prototyping instances are incompatible with the base `nvidia-container-toolkit` , it is not installed by default. 

  To use docker in a production instance, you will need to disable the /etc/docker/daemon.json override which activates the "thunder" runtime and install the standard nvidia container toolkit.
</Warning>
