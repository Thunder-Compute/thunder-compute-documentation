...
Note: make sure you install the cuda-compatible version of these libraries, just like you would with a physical GPU e.g., `tensorflow[and-cuda]`. The cuda-compatible PyTorch binary is pre-installed on every Thunder Compute instance.
...
Mining, staking, or otherwise interacting with cryptocurrency is strictly prohibited on Thunder Compute. If cryptocurrency-related activity is detected, the associated account is immediately banned from Thunder Compute and any credit is revoked. The account is then billed for the full amount of usage. 
